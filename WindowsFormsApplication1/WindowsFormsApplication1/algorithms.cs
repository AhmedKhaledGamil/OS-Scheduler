...
namespace CPUScheduler
{
    class Algorithms
    {
        ...
        {
            /*LinkedList<process> sorted_processes = new LinkedList<process>();
            List<process> SortedBurst = p.OrderBy(b => (b.getArrival())).ToList();
            double totalburst = 0.0;
            foreach (var item in SortedBurst) //copying arranged list in linked list
            {
                totalburst += item.getBurst();
            ...
            LinkedList<process> zero = new LinkedList<process>();
            for (double time = 0.0; time <= totalburst; time++)
            {
                SortedBurst[i].setBurst((SortedBurst[i].getBurst()) - 1);
                if (SortedBurst[i].getBurst() == 0)
                {
                    zero.AddLast(SortedBurst[i]);
                    SortedBurst.RemoveAt(i);
                    var--;
                }
                for (int k = 0; k < var - 1; k++)
                {
                    ...
                    {
                        if (SortedBurst[j].getBurst() > SortedBurst[j + 1].getBurst())
                        {
                            process temp = SortedBurst[j];
                            SortedBurst[j] = SortedBurst[j + 1];
                            SortedBurst[j + 1] = temp;
                        }
                    }
                }
                if (SortedBurst[i] != old)
                {
                    SortedBurst[i].setBurst((SortedBurst[i].getBurst()) - time);
                    zero.AddLast(SortedBurst[i]);
                }
            }
